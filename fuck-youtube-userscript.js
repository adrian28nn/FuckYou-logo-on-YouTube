...
(function() {
    ...
    // Function to get the current header color
    function getHeaderColor() {
        const header = document.querySelector('#background.ytd-masthead');
        return header ? window.getComputedStyle(header).backgroundColor : '';
    }

    // Function to get the appropriate fill color based on the header color
    function getCurrentFillColor() {
        const headerColor = getHeaderColor();
        if (headerColor === 'rgb(15, 15, 15)') { // Dark header color
            return darkModeColor;
        }
        if (headerColor === 'rgb(255, 255, 255)') { // Light header color
            return isVideoPage() ? videoPageColor : lightModeColor;
        }
    ...
    // Observe theme (light/dark mode) changes by watching changes to the header color
    function observeThemeChanges() {
        const header = document.querySelector('#background.ytd-masthead');
        new MutationObserver(() => {
            updateLogo(); // Update logo when the header color changes
